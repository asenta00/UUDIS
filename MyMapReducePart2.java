...
// programski kod u ovom dokumentu je izmijenjena verzija onoga sa http://www.javaworld.com/javaworld/jw-09-2008/hadoop-list2.txt 

import java.io.*; 
import java.util.*; 

public class MyMapReducePart2 
{ 
	...
	{ 
		...
		{ 
			...
			for (int i = 1; i <= m; i++) 
			{ 
				temp.add((String)values.get(count)); // dohvati ime prvog dokumenta i spremi ga u temp 
				count++; 
			} 

			buckets.add(temp); // u bucket ubaci ime prvog dokumenta, pa drugog 
			temp = new ArrayList(); 
		} 
	...
	{ 
		...
		{ 
			...
			new StartThread(elementList).start(); // kreiranje nove instance klase StartThread 
		} 
	...
	{ 
		...
		{	 
			...
			// Hashtable mapira keys u values 
