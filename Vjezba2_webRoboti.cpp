...
boost::regex url_regex("(http)://(www)\\.([a-z]+[a-z0-9%\\-]*)\\.(hr|com|net|edu|org|aspx|html)(((/[a-z0-9]+[a-z0-9%\\-]*((\\.(hr|com|net|edu|org|aspx|html))?))*)?)", boost::regbase::normal | boost::regbase::icase);
boost::regex word_regex("(znanost|fesb|split)", boost::regbase::normal | boost::regbase::icase);	// rijec koju trazimo po linkovima
...
void *printMessage (void *thread_id)
{
	...
	cout << "Link za nit broj " << thread_id_2 << ": " << lista_pet_linkova[thread_id_2] << endl;
...
int main()
{
	...
	// kod greske do koje je doslo prilikom izvrsavanja funkcije ili CURLE_OK ako nije doslo do greske
	CURLcode curl_code;
	...
	if (curl_handle)
	{
		...
		curl_easy_setopt(curl_handle, CURLOPT_URL, "http://www.unist.hr");
	...
	cout << "Ukupan broj trazenih rijeci je: " << brTrazenihRijeci << endl;
